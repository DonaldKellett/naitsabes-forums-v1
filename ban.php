...
<html>
	<head>
		<title>Naitsabes Forums - Ban User</title>
		<meta charset="utf-8" />
                        ...
                        while ($demote_user = $demote_user_query->fetch_assoc()) {
                          if ($_COOKIE['rank'] > $demote_user['rank']) {
                            if ($demote_user['rank'] > 1) {
                              if ($conn->query("UPDATE accounts SET rank = 1 WHERE username = '" . $demote_user['username'] . "'") == true) { ?>
                                <h2>User Ban Successful</h2>
                                <p>
                                  The ban was successful and the user you requested is now banned.
                                </p>
                              <?php } else { ?>
                                <h2>User Ban Failed</h2>
                                <p>
                                  Sorry, the user ban failed for some reason.  We apologise for any inconvenience caused.
                                </p>
                              <?php }
                            } else { ?>
                              <h2>User Already Banned</h2>
                              <p>
                                The user is already banned, no need to ban him/her again! ;)
                              </p>
                            <?php }
                          } else { ?>
                            <h2>Action Not Permitted</h2>
                            <p>
                              Sorry, you cannot ban a user that has the same rank or has a higher rank than you.
                            </p>
                          <?php }
                        }
                      } else { ?>
